...
Sequence::Sequence() { this->basesConcat = ""; }
void Sequence::setComplete(bool n_complete) { this->complete = n_complete; }
bool Sequence::getComplete() { return complete; }
...
std::string Sequence::getBasesConcat() { return this->basesConcat; }
void Sequence::updateStruct() {
  ...
  for (; itL != bases.end(); itL++) {
    ...
    for (int i = 0; i < auxLenght; i++) {  // Dependiendo el tamaño itera
...
int Sequence::countDifBases() {
  int countDif = 0;
  for (int i = 0; i < countBases.size(); i++) {
    if (countBases.at(i) != 0 && nitrogens.at(i) != 'X' &&
        nitrogens.at(i) != '-') {
      countDif++;
    }
  }
